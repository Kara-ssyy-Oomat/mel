...
async function loadAgentsProfitReport() {
  ...
  try {
    ...
    agentsSnapshot.forEach(doc => {
      agents[doc.id] = { id: doc.id, ...doc.data(), orders: [], totalSum: 0, totalProfit: 0, ordersCount: 0 };
    });
    ...
    ordersSnapshot.forEach(doc => {
      ...
      if (agents[partnerId]) {
        agents[partnerId].orders.push({ id: doc.id, ...data });
        agents[partnerId].totalSum += data.total || 0;
        agents[partnerId].totalProfit += orderProfit;
        agents[partnerId].ordersCount++;
        totalOrdersCount++;
        totalSum += data.total || 0;
        totalProfit += orderProfit;
        
        console.log(`📋 Заказ для агента ${agents[partnerId].name}:`, {
          клиент: data.name,
          сумма: data.total,
          прибыль: orderProfit
        });
