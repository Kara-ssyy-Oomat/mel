      ...
      <div style="background:#f8f9fa; border-radius:12px; padding:15px; border-left:4px solid ${statusColor};">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px; flex-wrap:wrap; gap:10px;">
          <div style="flex:1; min-width:200px;">
            <div style="font-weight:600; color:#333; font-size:15px;">${order.name || 'Клиент'}</div>
...
function updateAgentStats(orders) {
  let totalSum = 0;
  ...
  document.getElementById('agentTotalClients').textContent = uniqueClients.size;
  
  // Показываем изменения за период (если не "за все время")
  const periodFilter = document.getElementById('agentPeriodFilter')?.value || 'all';
  if (periodFilter !== 'all') {
    const prevPeriodOrders = getPreviousPeriodOrders(allAgentOrders, periodFilter);
    const prevSum = prevPeriodOrders.reduce((sum, o) => sum + (o.total || 0), 0);
    const prevProfit = Math.round(prevSum * 0.02);
    const prevClients = new Set(prevPeriodOrders.map(o => o.phone).filter(p => p)).size;
    
    const ordersChange = totalOrders - prevPeriodOrders.length;
    const profitChange = totalProfit - prevProfit;
    const clientsChange = uniqueClients.size - prevClients;
    
    document.getElementById('agentOrdersChange').textContent = 
      ordersChange > 0 ? `↑ +${ordersChange}` : ordersChange < 0 ? `↓ ${ordersChange}` : '→ 0';
    document.getElementById('agentProfitChange').textContent = 
      profitChange > 0 ? `↑ +${profitChange} сом` : profitChange < 0 ? `↓ ${profitChange} сом` : '→ 0';
    document.getElementById('agentClientsChange').textContent = 
      clientsChange > 0 ? `↑ +${clientsChange}` : clientsChange < 0 ? `↓ ${clientsChange}` : '→ 0';
  } else {
    document.getElementById('agentOrdersChange').textContent = '—';
    document.getElementById('agentProfitChange').textContent = '—';
    document.getElementById('agentClientsChange').textContent = '—';
  ...
  if (typeof Chart !== 'undefined') {
    ...
    canvas.parentElement.innerHTML = `
      <div style="padding:20px; text-align:center; color:#666;">
        <div>График недоступен</div>
        <div style="font-size:14px; margin-top:5px;">Подключите Chart.js для визуализации</div>
      </div>
    `;
  }
}
